...
def test_secrets_marked_sensitive():
    """Every user_config key whose env var name, title or description mentions a credential is sensitive, and nothing else is."""
    manifest = json.loads((REPO / "manifest.json").read_text())
    ...
    for var, value in env.items():
        ...
        cfg = manifest["user_config"][m.group(1)]
        text = (var + " " + cfg.get("title", "") + " " + cfg.get("description", "")).upper()
